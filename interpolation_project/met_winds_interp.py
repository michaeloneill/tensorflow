...
from plotting import get_long_lat_from_weather_db, plot_wind_map_ground_vs_preds, plot_locations_map
...
def test_model(X_test, y_test, locations_test, bbox, params, model, sess, n_plots, results_dir):
    ...
    plot_wind_map_ground_vs_preds(y_test[:, None, :], predictions[:, None, :],
                                  locations_test, bbox,
                                  n_plots, results_dir,
                                  scale=2.0,
                                  scale_units='inches')
def main():
    ...
    params = {
        'mlp': params_mlp,
        'train': params_train,
        'inpt_shape': {'x': [None, 11*2], 'y_': [None, 2]},
        'device':'/gpu:1',
        'results_dir': results_dir
    }

    data_filename = '../../data/interpolation_project/met_winds_interp_dataset_reshaped_scaled.npz'
