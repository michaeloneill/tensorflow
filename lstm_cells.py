...
class BNLSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    modelled on BasicLSTMCell https://github.com/tensorflow/tensorflow/
    blob/master/tensorflow/python/ops/rnn_cell.py'''

    def __init__(self, num_units, is_training, forget_bias=1.0):
        self._num_units = num_units
        self._is_training = is_training
        self._forget_bias = forget_bias  # control initial forget rate
    ...
    def __call__(self, x, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            '''
            Change bias argument to False since batch_norm
            will add bias via shift.
            input-hidden and hidden-hidden weights built as one variable,
            to be split into portions for each gate.
            '''

            concat = tf.nn.rnn_cell._linear(
                [x, h], 4*self._num_units, bias=False)
            ...
            new_h = (tf.tanh(batch_norm(
                new_c, 'new_h/', self._is_training)) * tf.sigmoid(o))
...
class ConvRNNCell(object):
    ...
    """
    Abstract object analagous to RNNCell but for a
    Convolutional RNN cell.
    Adapted from https://github.com/loliverhennigh/
    Convolutional-LSTM-in-Tensorflow/blob/master/BasicConvLSTMCell.py
    """
    ...
    def zero_state(self, b_size, dtype):
        ...
        Returns:
          tensor of shape [b_size x shape[0] x shape[1] x
        num_output_feature_maps]
          filled with zeros
        """
        shape = self.shape
        num_output_feature_maps = self.num_output_feature_maps
        zeros = (tf.zeros([b_size, shape[0],
                           shape[1], num_output_feature_maps]),
                 tf.zeros([b_size, shape[0],
                           shape[1], num_output_feature_maps]))
...
class ConvLSTMCell(ConvRNNCell):
    ...
    Adapted from https://github.com/loliverhennigh/
    Convolutional-LSTM-in-Tensorflow/blob/master/
    BasicConvLSTMCell.py
    """

    def __init__(self, shape, filter_size,
                 num_output_feature_maps, forget_bias=1.0):
        """
...
class BNConvLSTMCell(ConvRNNCell):
    ...
    def __init__(self, shape, filter_size,
                 num_output_feature_maps, is_training, forget_bias=1.0):
        """
        Initialize the basic Conv LSTM cell.
        Args:
        shape: int tuple that is height and width of cell
    ...
    def __call__(self, x, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            new_h = (tf.tanh(
                batch_norm(
                    new_c, 'new_h/', self._is_training, conv=True))
                     * tf.sigmoid(o))
...
def _conv_linear(args, filter_size, num_output_feature_maps, bias, bias_start=0.0, scope=None):
    ...
    convolution:
    Args:
      args: list of 4D Tensors b x H x W x num_input_feature_maps (e.g. [x,h])
    ...
    for shape in shapes:
        if len(shape) != 4:
            raise ValueError("Linear is expecting 4D arguments")
        if not shape[3]:
            raise ValueError("Linear expects shape[3] of arguments")
        else:
    ...
    with tf.variable_scope(scope or "Conv"):
        filtr = tf.get_variable(
            ...
            [filter_size[0], filter_size[1],
             total_input_depth, num_output_feature_maps],
            initializer=tf.truncated_normal_initializer(),
            dtype=dtype
        )
        if len(args) == 1:
            res = tf.nn.conv2d(args[0], filtr,
...
def batch_norm(x, name_scope, is_training, epsilon=1e-3,
               decay=0.999, conv=False):
    ...
    with tf.variable_scope(name_scope):
        ...
        train_mean_op = (tf.assign(pop_mean, pop_mean * decay +
                                   batch_mean * (1 - decay)))
        train_var_op = (tf.assign(pop_var, pop_var * decay +
                                  batch_var * (1 - decay)))
