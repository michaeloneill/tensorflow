...
def get_train_time_patches_labels(images_train, images_val, images_test, params):
    ...
    # [n_patches_train x n_channels] values [0,255)
    indices_train = np.digitize(X_train[:, params['p_i'], params['p_j'], :], bins, right=True)-1 
    indices_val = np.digitize(X_val[:, params['p_i'], params['p_j'], :], bins, right=True)-1
    indices_test = np.digitize(X_test[:, params['p_i'], params['p_j'], :], bins, right=True)-1

    y_train = np.zeros(shape=(params['n_patches_train'], params['n_bins'], params['n_channels']))
    y_val = np.zeros(shape=(params['n_patches_val'], params['n_bins'], params['n_channels']))
    y_test = np.zeros(shape=(params['n_patches_test'], params['n_bins'], params['n_channels']))

    for i in xrange(params['n_patches_train']):
        for j in xrange(params['n_channels']):
    ...
    y_train = y_train.reshape(-1, params['n_bins']*params['n_channels'])
    y_val = y_val.reshape(-1, params['n_bins']*params['n_channels'])
    y_test = y_test.reshape(-1, params['n_bins']*params['n_channels'])

    # now mask patches
    
    X_train = mask_input(X_train, params['p_i'], params['p_j'])
    X_val = mask_input(X_val, params['p_i'], params['p_j'])
    X_test = mask_input(X_test, params['p_i'], params['p_j'])
...
def generate_wind_train_time_dataset(unroll, i_filename, o_filename):
    ...
    for i, j in enumerate(['x', 'y']):
        ...
        wind_images.save('wind_{}.png'.format(j))
...
if __name__=='__main__':
    ...
    # o_filename_train = '../../data/generate_weather_project/wind/wind_201401_train_time_dataset_pixel_mlp.npz' # mlp
    # generate_wind_train_time_dataset(True, i_filename, o_filename_train)

    # o_filename_train = '../../data/generate_weather_project/wind/wind_201401_train_time_dataset_pixel_cnn.npz' # cnn
    # generate_wind_train_time_dataset(False, i_filename, o_filename_train)

    # o_filename_train_overfit = '../../data/generate_weather_project/wind/wind_201401_train_time_dataset_pixel_mlp_overfit.npz' # mlp
    # generate_wind_train_time_dataset(True, i_filename, o_filename_train_overfit)

    # o_filename_train_overfit = '../../data/generate_weather_project/wind/wind_201401_train_time_dataset_pixel_cnn_overfit.npz' # cnn
    # generate_wind_train_time_dataset(False, i_filename, o_filename_train_overfit)
